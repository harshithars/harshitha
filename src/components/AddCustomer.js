    ...
    const customerElement = (
        ...
        {errMsg && <h3 className = "alert alert-danger">{errMsg}</h3>}
            ...
            <form onSubmit={handleSubmit}>
                ...
                <div className="form-group">
                <input type="email" name="email"  className="form-control col-4" value={newCustomer.email} onChange={(event) => handleChange(event)} placeholder="EmailId" required></input><br />
                </div>
                <div className="form-group">
                <input type="password" name="password"  className="form-control col-4" value={newCustomer.password} onChange={(event) => handleChange(event)} placeholder="CustomerPassword"  required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" title="Must contain at least one  number and one uppercase and lowercase letter, and at least 8 or more characters"></input><br />
                </div>
                <div className="form-group">
